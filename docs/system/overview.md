...
### Decision 1: One validation system, one error-response envelope

**Context**: The codebase previously carried two validation stacks simultaneously — `class-validator`/`class-transformer` DTOs driven by a global Nest `ValidationPipe`, and `nestjs-zod`'s `ZodValidationPipe`/`ZodSerializerInterceptor` registered globally as `APP_PIPE`/`APP_INTERCEPTOR`. Four incompatible error body shapes reached the client depending on which component handled the failure.

**Decision**: `nestjs-zod` was removed from the HTTP pipeline and from `package.json`. `class-validator`'s `ValidationPipe` (`src/shared/utils/validation-pipe.config.ts`) is now the sole validation system, registered as `APP_PIPE` in `src/shared/modules/base.module.ts:52-55`. The two former exception filters (`ExternalExceptionFilter`, `PrismaClientExceptionFilter`) were collapsed into one `GlobalExceptionFilter` (`@Catch()`, no argument, `src/shared/filters/global-exception.filter.ts`) dispatching to pure mappers, so every error response now shares one envelope (`statusCode`, `error`, `message`, `details`, `requestId`). `src/main.ts` no longer configures any pipe or filter — both are `BaseModule` providers, which is what lets the e2e harness (`test/e2e/support/create-test-app.ts`) get production's exact error contract by importing `AppModule` alone.

**Rationale**: A single validation system and a single response writer mean the response shape can no longer depend on `APP_FILTER`/`APP_PIPE` registration order or on which library happened to throw. See `docs/error-handling.md` for the full contract, the pieces that build it, and how to add a new error. Standalone `zod` remains in use inside `GoogleService` to parse the OAuth `state` blob (`src/routes/auth/google.service.ts`) — that is not HTTP validation and is unaffected by this decision.
...
- **API Security**: Global `ValidationPipe` (`whitelist: true`, `forbidNonWhitelisted: true`, `transform: true`), the sole HTTP validation system, registered as `APP_PIPE` in `base.module.ts:52-55` (`src/shared/utils/validation-pipe.config.ts`). CORS is hardcoded to a single origin `http://localhost:3000` with no env override (`main.ts:20-25`) — **will silently block/allow only that origin in every environment including production** unless changed before deploy. No rate-limiting or `@nestjs/throttler` dependency found in `package.json`. No global API prefix (`docs/google-oauth-login-flow.md:419`).
