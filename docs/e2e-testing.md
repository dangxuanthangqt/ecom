...
| Concern  | Choice                                                                                                                                                                                        | Why                                                                                                                                                                                                                                                 |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Database | A separate `ecom_e2e` database on the same Postgres instance `docker-compose.yml` already runs, rebuilt with `prisma migrate reset --force` (Prisma 7 never seeds on reset) before every run. | No testcontainers, no second compose service — reuses the existing `db` container. `scripts/prepare-e2e-database.sh` refuses to run unless `DATABASE_URL` contains `ecom_e2e`, so a stale or typo'd env can never point the reset at dev/prod data. |
...
| Env file | `.env.test` is the whole e2e environment, not a set of overrides: each `NODE_ENV` maps to exactly one file and nothing falls through from `.env`. Copy it from `.env.test.example`.           | A partial file would let e2e silently inherit dev or prod values; `validateEnv` now fails the boot instead, naming the missing key.                                                                                                                 |
