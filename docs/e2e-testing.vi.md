...
| Concern  | Choice                                                                                                                                                                                                           | Why                                                                                                                                                                                                                                                                               |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Database | Một database `ecom_e2e` riêng, nằm trên cùng instance Postgres mà `docker-compose.yml` đã chạy sẵn, được rebuild bằng `prisma migrate reset --force` (Prisma 7 không bao giờ seed khi reset) trước mỗi lần chạy. | Không cần testcontainers, không cần thêm compose service — tái dùng luôn container `db` có sẵn. `scripts/prepare-e2e-database.sh` từ chối chạy nếu `DATABASE_URL` không chứa `ecom_e2e`, nên một env cũ hay gõ nhầm không bao giờ có thể khiến lệnh reset đụng vào data dev/prod. |
...
| File env | `.env.test` là toàn bộ môi trường e2e, không còn là file override: mỗi `NODE_ENV` ứng với đúng một file và không có gì fallback từ `.env`. Copy từ `.env.test.example`.                                          | File thiếu key sẽ khiến e2e âm thầm dùng giá trị của dev/prod; giờ `validateEnv` fail ngay lúc boot và chỉ đúng key bị thiếu.                                                                                                                                                     |
